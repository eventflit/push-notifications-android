package com.eventflit.pushnotifications.lint;
...
public final class WrongPushNotificationsUsageDetector extends Detector implements Detector.UastScanner {
    private static final String INCORRECT_MESSAGE_FORMAT = "The app id argument looks incorrect. It should be something like '%s'.";
    private static final int ISSUE_BAD_APP_ID_PRIORITY = 10;
    private static final Issue ISSUE_BAD_APP_ID =
            Issue.create(
                    "BadPushNotificationsAppId",
                    "Checks the App Id argument for Push Notifications",
                    "The App Id for Push Notifications follows a specific format, so unrecognizable values are likely to be wrong.",
                    Category.MESSAGES,
                    ISSUE_BAD_APP_ID_PRIORITY,
                    Severity.WARNING,
    ...
    public List<String> getApplicableConstructorTypes() {
        return Collections.singletonList("com.eventflit.pushnotifications.PushNotificationsInstance");
    }
    ...
    public void visitConstructor(JavaContext context, UCallExpression call, PsiMethod method) {
        reportBadAppIdIssue(context, call);
    }
    ...
    public void visitMethod(JavaContext context, UCallExpression call, PsiMethod method) {
        ...
        if (evaluator.isMemberInClass(method, "com.eventflit.pushnotifications.PushNotifications")) {
    ...
    private static void reportBadAppIdIssue(JavaContext context, UCallExpression call) {
        List<UExpression> arguments = call.getValueArguments();

        if (arguments.size() == 2) {
            Object possibleAppId = arguments.get(1).evaluate();
            if (possibleAppId != null && possibleAppId != "") {
                try {
                    UUID.fromString(possibleAppId.toString());
                } catch (Exception e) {
                    LintFix.GroupBuilder fixGrouper = fix().group();
                    LintFix fix = fixGrouper.build();

                    String message = String.format(INCORRECT_MESSAGE_FORMAT, UUID.randomUUID());
                    context.report(ISSUE_BAD_APP_ID, call, context.getLocation(arguments.get(1)), message, fix);
                }
            }
        }
    }
